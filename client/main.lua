...
function WeaponStatus()
    ...
    if IsPedArmed(player, 7) then
        ...
        for key, value in pairs(AllWeapons) do
            ...
            for keyTwo, valueTwo in pairs(AllWeapons[key]) do
                if weapon == GetHashKey('weapon_' .. keyTwo) then
                    ...
                    if key == 'melee' then
                        status['ammo'].enabled = false
                    else
                        if keyTwo == 'stungun' then
                            status['ammo'].enabled = false
                        else
                            status['ammo'].enabled = true
                        end
                    end
        ...
        status['ammo'].amount = ammoRemaining
        status['ammo'].clip = ammoClip
    else
        status['armed'] = false
        status['ammo'].enabled = false
    end
...
function PlayerStatus()
    ...
    playerStatus['status'][showPlayerStatus] = {
        name = 'health',
        Maxvalue = GetEntityMaxHealth(ped) - 100,
    ...
    playerStatus['status'][showPlayerStatus] = {
        name = 'armour',
        Maxvalue = GetPlayerMaxArmour(PlayerId()),
        value = GetPedArmour(ped),
    }

    TriggerServerEvent('d7y_destinyHUD:get')
...
RegisterNetEvent("d7y_destinyHUD:set:hunger:thirst")
AddEventHandler("d7y_destinyHUD:set:hunger:thirst", function(data)
